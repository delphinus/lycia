...
func (c Cache) LoadCache() (err error) {
	...
	if err != nil {
		err = LyciaError(fmt.Sprintf("access error to cache path '%s': %s", CachePath, err))
		return
	}
	...
	if err != nil {
		err = LyciaError(fmt.Sprintf("cache path '%s' is corrupted: %s", CachePath, err))
		return
	}
...
func (c Cache) SaveCache() (err error) {
	...
	if err != nil {
		err = LyciaError(fmt.Sprintf("cannot encode cache to JSON: %s", err))
		return
	}

	err = ioutil.WriteFile(CachePath, byt, 0644)
	if err != nil {
		err = LyciaError(fmt.Sprintf("cannot write cache to file '%s': %s", CachePath, err))
	}
